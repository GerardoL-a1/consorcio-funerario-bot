...
def enviar_log_por_whatsapp(error_texto):
    try:
        mensaje = f"🚨 *Error Crítico Detectado:*
{error_texto[:1500]}\n\n(Los detalles completos están en logs/app_log.txt)"
        requests.post(TWILIO_MESSAGING_URL, auth=TWILIO_AUTH, data={
            "To": NUMERO_REENVIO,
            "From": "whatsapp:+14155238886",
            "Body": mensaje
        })
        log_info("✅ Log de error crítico enviado por WhatsApp.")
    except Exception as e:
        log_error(f"❌ No se pudo enviar el log crítico por WhatsApp: {str(e)}")
...
if not TWILIO_ACCOUNT_SID or not TWILIO_AUTH_TOKEN:
    raise ValueError("❌ TWILIO_ACCOUNT_SID o TWILIO_AUTH_TOKEN no están configurados correctamente.")
...
try:
    if os.path.exists("credenciales.json"):
        ...
        credenciales = ServiceAccountCredentials.from_json_keyfile_dict(json.loads(json_creds), alcance)
...
def registrar_interaccion(numero, mensaje, origen="Cliente"):
    log_info(f"Interacción de {origen} ({numero}): {mensaje}")
    log_info(f"Interacción de {origen} ({numero}): {mensaje}")
    fecha = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    if hoja: hoja.append_row([fecha, numero, origen, mensaje])
...
def webhook():
    ...
    log_error(f"❌ Error inesperado en webhook: {str(e)}")
    return responder("⚠️ Ocurrió un error interno. Escribe *menú* para volver a intentar.")
except Exception as e:
    log_error(f"❌ Error inesperado en webhook: {str(e)}")
    enviar_log_por_whatsapp(str(e))
    return responder("⚠️ Ocurrió un error interno. Estamos trabajando para solucionarlo. Escribe *menú* para intentar de nuevo.")
    ...
    if estado.get("menu") == "emergencia":
        ...
        log_info(f"Mensaje de EMERGENCIA reenviado: {alerta}
        })
        sesiones[telefono] = {}
    ...
    if estado.get("menu") == "cita":
        ...
        log_info(f"Mensaje de CITA reenviado: {datos}
        })
        sesiones[telefono] = {}
